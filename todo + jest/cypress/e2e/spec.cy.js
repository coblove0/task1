describe('E2E Tests', () => {
  ...
  it('Добавление задач и фильтрация', () => {
    ...
    cy.get('[data-testid="todo-list"]', { timeout: 10000 }).should('contain.text', 'Активная задача 1');
    cy.get('[data-testid="todo-list"]').should('contain.text', 'Активная задача 2');
    cy.get('[data-testid="todo-list"]').should('not.contain.text', 'Выполненная задача 1');

    // Проверка фильтра "Только выполненные"
    cy.get('[data-testid="filter-completed"]').click();
    cy.get('[data-testid="todo-list"]', { timeout: 10000 }).should('contain.text', 'Выполненная задача 1');
    cy.get('[data-testid="todo-list"]').should('not.contain.text', 'Активная задача 1');
    cy.get('[data-testid="todo-list"]').should('not.contain.text', 'Активная задача 2');
  });
})